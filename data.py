...
def Graph_load_batch(min_num_nodes = 20, max_num_nodes = 1000, name = 'Amazon',node_attributes = True,graph_labels=True):
    '''
...
def encode_adj(labels, attributes, adj, max_prev_node=10, is_full = False):
    '''
    ...
    attributes_output = np.zeros((adj.shape[0],attributes.shape[1]))
    labels_output = np.zeros(adj.shape[0])
    for i in range(adj.shape[0]):
        ...
        attributes_output[i,:] = attributes[i,:]
        labels_output[i] = labels[i]
 
    return adj_output, attributes_output, labels_output

def encode_adj_with_rep(adj_encoded,att_encoded):
    rep_col = adj_encoded.shape[1] + att_encoded.shape[1]
    rep_encoded = np.zeros((adj_encoded.shape[0],rep_col))
    for i in range(rep_encoded.shape[0]):
        rep_encoded[i,0:adj_encoded.shape[1]] = adj_encoded[i,:]
        rep_encoded[i,adj_encoded.shape[1]:] = att_encoded[i,:]
    return rep_encoded
...
class Graph_sequence_sampler_pytorch(torch.utils.data.Dataset):
    def __init__(self, G_list, max_num_node=None, max_prev_node=None, iteration=20000):
        self.adj_all = []
        self.len_all = []
        self.nodes_attribute = []
        self.labels = []
        self.G_list = G_list
        for G in G_list:
            self.adj_all.append(np.asarray(nx.to_numpy_matrix(G)))
            self.len_all.append(G.number_of_nodes())            
            attributes_dic = nx.get_node_attributes(G,'feature')
            attributes_temp = []
            for _,value in attributes_dic.items():
                attributes_temp.append(value)
            attributes_temp = np.asarray(attributes_temp)
            self.nodes_attribute.append(attributes_temp)
            
            labels_dic = nx.get_node_attributes(G,'label')
            labels_temp = []
            for _,value in labels_dic.items():
                labels_temp.append(value)
    ...
    def __getitem__(self, idx):
        ...
        attributes_copy = self.nodes_attribute[idx].copy()
        labels_copy = self.labels[idx].copy()
        batch_col = self.max_prev_node + attributes_copy.shape[1]
        x_batch = np.zeros((self.n, batch_col))  # here zeros are padded for small graph
        x_batch[0,:] = 1 # the first input token is all ones
        y_batch = np.zeros((self.n, self.max_prev_node))  # here zeros are padded for small graph
        att_batch = np.zeros((self.n,attributes_copy.shape[1])) # zeros are padded for attributes of nodes in small graphs
        label_batch = -1*np.ones(self.n)
        # generate input x, y pairs
        len_batch = self.max_prev_node + attributes_copy.shape[1]
        x_idx = np.random.permutation(adj_copy.shape[0])
        adj_copy = adj_copy[np.ix_(x_idx, x_idx)]
        adj_copy_matrix = np.asmatrix(adj_copy)
        G = nx.from_numpy_matrix(adj_copy_matrix)
#         print('length of G with index ', idx, ': ',len(G), ', length of G_list[idx] with index ',idx,': ',len(self.G_list[idx]))
        # then do bfs in the permuted G
        start_idx = np.random.randint(adj_copy.shape[0])
        x_idx = np.array(bfs_seq(G, start_idx))
        adj_copy = adj_copy[np.ix_(x_idx, x_idx)]
        adj_encoded, att_encoded, labels_encoded = encode_adj(labels_copy.copy(), attributes_copy.copy(), adj_copy.copy(), max_prev_node=self.max_prev_node)
        # get x and y and adj
        # for small graph the rest are zero padded
        y_batch[0:adj_encoded.shape[0], :] = adj_encoded        
        x_encoded = encode_adj_with_rep(adj_encoded,att_encoded)
        x_batch[0:adj_encoded.shape[0], :] = x_encoded
        att_batch[0:adj_encoded.shape[0],:] = att_encoded
        label_batch[0:adj_encoded.shape[0]] = labels_encoded
        return {'x':x_batch,'y':y_batch, 'att':att_batch, 'label':label_batch, 'len':len_batch}
